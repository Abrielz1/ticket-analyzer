...
public class ConsoleWriter {
    ...
    public void printResults(final Map<String, Duration> minFlightTimes, final BigDecimal priceDifference) {
        ...
        report.append(EOL).append("--- TICKET ANALYSIS RESULTS ---").append(EOL);
        report.append(EOL).append("Minimum flight time per carrier (Vladivostok -> Tel Aviv):").append(EOL);

        if (minFlightTimes.isEmpty()) {
            report.append(EOL).append(System.lineSeparator());
            report.append(EOL).append(" No direct or connecting flights found for any carrier.").append(EOL);

        } else {
            minFlightTimes.forEach((carrier, duration) ->
                    report.append(String.format("    - Carrier '%s': %d hours %d minutes%n",
                            carrier,
                            duration.toHours(),
                            duration.toMinutesPart()))
            );
            report.append(EOL).append("Price analysis (Vladivostok -> Tel Aviv):").append(EOL);
            BigDecimal displayDifference = priceDifference.setScale(2, RoundingMode.HALF_UP);
            report.append(String.format("    Difference between average and median price: %s RUB%n", displayDifference));
